...
/*
These functions always take at least one argument, and the first argument is
always a string representing the name of the database
model: 'minions', 'ideas', 'meetings', or 'work'.
*/

/*takes only single argument for model name; returns array of elements in
database or null if invalid argument supplied*/
...
/*takes the model name argument + second string argument that is a
unique ID of the element.  Return instance with valid inputs or -1 with
invalid id*/
...
/*Takes the model name argument and a second argument which is an object with
the key-value pairs of the new instance. 'addToDatabase' handles assigning
.id properties to the instances.  It does not check to make sure that valid
inputs are supplied, so you will have to add those checks to your routes if
necessary.  'addToDatabase' will return a newly-created instance from the database.
This function will validate the schema of the instance to create and throw
an error if it is invalid.  */
//have to add checks to routes to make sure this is valid
...
This function will validate the schema of the updated instance and throw an
...
unique ID of the element to delete.  Returns 'true'  if the delete occurs
...
/*Takes only the single argument for model name.  Deletes all elements from the
proper model and returns a new, empty array.  You will only need to use this
